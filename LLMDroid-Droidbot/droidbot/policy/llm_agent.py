#LLMDroid-Droidbot/droidbot/policy/llm_agent.py
import os.path
import sys
...
class LLMAgent:
    ...
    MODEL_STR = 'llama-3.3-70b-versatile'  # default, overridden by config
    ...
    def __init__(self, app: 'App', utg: 'UTG'):
        ...
        with open('./config.json', mode='r', encoding='utf-8') as file:
            ...
            # Use Model and BaseUrl from config if provided
            if 'Model' in config and config['Model']:
                LLMAgent.MODEL_STR = config['Model']
            base_url = config.get('BaseUrl', None)
            self.__start_prompt = f"I'm now testing an app called {self.__app_name} on Android.\n{self.__app_desc}\n"
        # init client
        client_kwargs = {'api_key': self.__api_key}
        if base_url:
            client_kwargs['base_url'] = base_url
        self.__client = OpenAI(**client_kwargs)
        self.__init_client()
